...
namespace rt::gfx {
    ...
    void Renderer::run(const RendererContext &context, FrameBuffer &fb) {
        ...
        vkCore.createBuffer(imageSize, vk::BufferUsageFlagBits::eTransferDst,
                            vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent,
                            stagingBuffer, stagingBufferMemory); {
            const auto cmd = vkCore.beginSingleTimeCommands();
            vkCore.transitionImageLayout(outputImage.image, vk::ImageLayout::eGeneral,
                                         vk::ImageLayout::eTransferSrcOptimal, *cmd);
            vkCore.endSingleTimeCommands(*cmd);
        }

        vkCore.copyImageToBuffer(outputImage.image, stagingBuffer, context.screenSettings->HEIGHT,
                                 context.screenSettings->WIDTH);
